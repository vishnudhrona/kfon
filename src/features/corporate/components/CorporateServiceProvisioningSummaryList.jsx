...
import { ACTION_TYPES, fetchEnquirySummaryWithCircuitProvisioning } from '../action';
import { getEnquirySummaryWithCircuitProvisioning } from '../selector';
...
const ServiceProvisioningCard = ({ data, index }) => {
    ...
    const handleNavigate = () => {
        router.navigate({
            to: '/app/corporate/service-provisioning-orders/$enquiryId',
            ...
            >
                <HStack w="full" spacing={3} align="center" position="relative">
                    ...
                    <VStack flex={1} minW={0} align="stretch" spacing={2}>
                        {/* Row 1: ID badge | Company + type | (verified) | spacer | Status */}
...
const CorporateServiceProvisioningSummaryList = () => {
    const { t } = useTranslation();
    const dispatch = useDispatch();
    const apiProgress = useSelector(getApiProgress);
    const summaryState = useSelector(getEnquirySummaryWithCircuitProvisioning);

    const isLoading = !!(apiProgress[ACTION_TYPES.FETCH_ENQUIRY_SUMMARY_WITH_CIRCUIT_PROVISIONING]) || summaryState?.isLoading;
    const list = summaryState?.data ?? [];

    useEffect(() => {
        dispatch(fetchEnquirySummaryWithCircuitProvisioning());
    }, [dispatch]);
...
export default CorporateServiceProvisioningSummaryList;

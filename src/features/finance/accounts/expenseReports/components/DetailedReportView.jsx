...
import { C, fmtINR, fmtINRFull, shortTitle } from './ExpenseReportsShared';
...
export default function DetailedReportView({
  ...
  const totalCol = schema.find((c) => c.total);
  const totalKey = totalCol ? totalCol.k : null;
  const grossFiltered = totalKey ? rows.reduce((s, x) => s + (+x[totalKey] || 0), 0) : 0;
  const gstFiltered = rows.reduce((s, x) => s + (+(x.totGst || x.gst || 0) || 0), 0);
  const taxableFiltered = rows.reduce((s, x) => s + (+(x.taxable || x.lnpBase || x.incentiveVal || 0) || 0), 0);
  const tdsFiltered = rows.reduce((s, x) => s + (+(x.tds || 0) || 0), 0);
      ...
      <Flex direction='column' gap='10px'>
        <Flex align='center' gap='10px' mb='2px' flexWrap='wrap'>
          ...
          <Text fontSize='11px' fontWeight='800' letterSpacing='1.2px' textTransform='uppercase' color={C.inkSoft}>
            {t('menu.expenseReport.detailSectionALabel')}
          </Text>
        ...
        <Box bg='white' border='1px solid' borderColor={C.line} borderRadius='14px' px='20px' py='14px'>
          <Grid templateColumns={{ base: '1fr', lg: '1.2fr auto auto' }} alignItems='center' gap='32px'>
            {/* Left Section */}
            <Box>
              <Text fontSize='10px' fontWeight='700' letterSpacing='0.8px' color={C.inkSoft}>
                {t('menu.expenseReport.netPayable')}
              </Text>

              <Text fontSize='36px' fontWeight='500' color={C.maroon800} lineHeight='1' mt='2px'>
                ₹{fmtINR(grossFiltered)}
              </Text>

              <Text fontSize='12px' color={C.inkSoft} mt='4px'>
                {t('menu.expenseReport.taxable')}: ₹{fmtINR(taxableFiltered)}
                &nbsp;·&nbsp; {t('menu.expenseReport.gst')}: ₹{fmtINR(gstFiltered)}
                &nbsp;·&nbsp; {t('menu.expenseReport.tds')}: ₹{fmtINR(tdsFiltered)}
              </Text>
            </Box>
            ...
            <Flex align='center' gap='20px'>
              <Box textAlign='center'>
                <Text fontSize='10px' fontWeight='500' color={C.inkSoft}>
              ...
              <Box>
                <Box h='6px' w='160px' borderRadius='999px' overflow='hidden' display='flex' bg={C.lineSoft}>
                  <Box bg={C.mint} w='75%' />
                  <Box bg={C.rose} w='25%' />
                </Box>
              </Box>
            ...
            <Flex gap='24px' align='center'>
              <Box textAlign='right'>
                <Flex justify='flex-end' align='center' gap='6px'>
                  ...
                  <Text fontSize='10px' fontWeight='700' color={C.inkSoft}>
                    {t('menu.expenseReport.netOfTds')}
                  </Text>
                </Flex>

                <Text fontSize='28px' fontWeight='500' color={C.mintDeep} lineHeight='1' mt='2px'>
                  ₹{fmtINR(grossFiltered - tdsFiltered)}
                </Text>
              </Box>
              ...
              <Box textAlign='right'>
                <Flex justify='flex-end' align='center' gap='6px'>
                  ...
                  <Text fontSize='10px' fontWeight='700' color={C.inkSoft}>
                    {t('menu.expenseReport.tdsWithheld')}
                  </Text>
                </Flex>

                <Text fontSize='28px' fontWeight='500' color={C.roseDeep} lineHeight='1' mt='2px'>
                  ₹{fmtINR(tdsFiltered)}
                </Text>
              </Box>
            </Flex>
      ...
      <Flex direction='column' gap='10px'>
        ...
        >
          ...
          >
            ...
            <Box
              as='input'
              placeholder={t('menu.expenseReport.searchAllPlaceholder')}
              value={filterQuery}
              onChange={(e) => {
              ...
              {pageRows.length === 0 ? (
                ...
                pageRows.map((row, idx) => (
                  <Box
                    ...
                    _hover={{ bg: C.roseBg }}
                    onClick={() => {
                      setSelectedRowIndex(start + idx);
                      setModalOpen(true);
                    }}
                  >
                    {schema.map((c) => (
                      <Box key={c.k} display='flex' justifyContent={c.num ? 'flex-end' : 'flex-start'}>

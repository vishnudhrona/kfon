...
import { fetchLnpPartnerStatusDropdown, updateAgnpPartner } from '../action';
import { getLnpPartnerStatusOptions } from '../selector';
import { partnerPreviewUpdate } from '../validation';

const AGNPFeasibilityPopup = ({ data, onClose }) => {
  ...
  useEffect(() => {
    dispatch(fetchLnpPartnerStatusDropdown({ type: 'agnp' }));
  }, [dispatch]);
  ...
  const {
    ...
    defaultValues: {
      status: data?.agnpStatus || '',
      remarks: data?.agnpRemarks || ''
    }
  });

  useEffect(() => {
    if (data) {
      const currentStatus = filteredStatusOptions?.find((item) => item?.id === data?.agnpStatus?.id);
      reset({
        status: currentStatus || data.agnpStatus || '',
        remarks: data.agnpRemarks || ''
      });
    }
  ...
  const onSubmit = async (formData) => {
    ...
    const currentStatusId = data?.agnpStatus?.id;
...
export default AGNPFeasibilityPopup;

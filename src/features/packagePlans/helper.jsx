...
export const packageSubmitMappings = [
  ...
  { key: 'status', target: 'status', includeIfExists: true, transform: (v) => (String(v) === 'true' ? 'Active' : 'Inactive') },
  ...
  { key: 'categoryType', target: 'categoryId', extract: 'id', includeIfExists: true },
...
export const corporatePackageSubmitMappings = [
  ...
  { key: 'serviceType', target: 'serviceType', extract: 'srvId', includeIfExists: true },
  { key: 'subServiceType', target: 'subServiceType', extract: 'subServiceId', includeIfExists: true },
...
export const formatCorporatePreviewData = (data) => {
  ...
  if (data) {
    response = [
      { label: t('serviceType'), value: get(data, 'serviceType.serviceName', t('notAvailable')) },
      { label: t('subServiceType'), value: get(data, 'subServiceType.serviceName', t('notAvailable')) },
      ...
      { label: t('fallBackSpeed'), value: get(data, 'fallbackSpeed.name', t('notAvailable')) },
...
export const formatSubServicedata = (data) => {
  const fields = [
    { label: t('name'), value: get(data, 'name', t('notAvailable')) },
    { label: t('label'), value: get(data, 'label', t('notAvailable')) },
    { label: t('sequence'), value: get(data, 'sequence', t('notAvailable')) }
  ];

  if (data?.discountable) {
    fields.push({ label: <TickTrueIcon color="#008232" boxSize={4} />, value: t('discounts') });
  }

  if (data?.partnerMapping) {
    fields.push({ label: <TickTrueIcon color="#008232" boxSize={4} />, value: t('provider') });
  }

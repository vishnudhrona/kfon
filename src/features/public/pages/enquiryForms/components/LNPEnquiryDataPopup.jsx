...
const LNPEnquiryDataPopup = ({ isOpen, setIsOpen, mobileEnquiryData, emailEnquiryData }) => {
    ...
    const EnquirySection = ({ data, messageKey }) => (
        <>
            <DataRow label={t('partnercompanyName')} value={data?.partnerCompanyName} />
            <DataRow label={t('contactName')} value={data?.partnerName} />
            <DataRow label={t('mobileNumber')} value={data?.partnerMobile} />
            <DataRow label={t('email')} value={data?.partnerEmail} />
    ...
    return (
        <Popup isOpen={isOpen} size='xl' placement='center' onOpenChange={setIsOpen}>
            ...
            <VStack p='20px 30px' gap={0} maxH='60vh' overflowY='auto' alignItems='stretch'>
                ...
                {isSameEnquiry ? (
                    <>
                        <DataRow label={t('partnercompanyName')} value={displayData?.partnerCompanyName} />
                        <DataRow label={t('contactName')} value={displayData?.partnerName} />
                        <DataRow label={t('mobileNumber')} value={displayData?.partnerMobile} />
                        <DataRow label={t('email')} value={displayData?.partnerEmail} />
                        <Text fontSize='18px' fontWeight={600} color='#272727' mt={6} mb={3}>
                            {t('enquiryInformation')}
                        </Text>
...
export default LNPEnquiryDataPopup;
